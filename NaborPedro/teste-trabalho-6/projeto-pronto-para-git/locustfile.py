...
class GrpcMusicUser(User):
    def on_start(self):
        ...
        self.stub = music_pb2_grpc.MusicCatalogServiceStub(self.channel)
    ...
    def catalogo_leitura(self):
        method, name, payload = next_item(
            self.catalog_counter,
            [
                (self.stub.ListUsers, "gRPC/listar-usuarios", music_pb2.EmptyRequest()),
                (self.stub.ListSongs, "gRPC/listar-musicas", music_pb2.EmptyRequest()),
                (
                    self.stub.ListUserPlaylists,
                    "gRPC/listar-playlists-usuario",
                    music_pb2.UserPlaylistQueryRequest(user_id=USER_ID),
                ),
                (
                    self.stub.ListPlaylistSongs,
                    "gRPC/listar-musicas-playlist",
                    music_pb2.PlaylistSongQueryRequest(playlist_id=PLAYLIST_ID),
                ),
                (
                    self.stub.ListSongPlaylists,
                    "gRPC/listar-playlists-musica",
                    music_pb2.SongPlaylistQueryRequest(song_id=SONG_ID),
                ),
            ],
        )
